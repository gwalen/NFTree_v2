...
async function main() {
  ...
  console.log("Call string transfer from L2 -> L1");
  const txMint = await l2Root.testStringTransfer(l1Wallet.address, "gw");
  await txMint.wait();
  ...
  const stringSent = await l1Root.getTransferredStringData();
  ...
  console.log("Check if L1 string array transfer for successful");
  const stringArraySent = await l1Root.getTransferredStringArrayData();
